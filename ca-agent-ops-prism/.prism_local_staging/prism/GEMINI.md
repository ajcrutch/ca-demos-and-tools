...
## OSS Exposure & Syncing

This project is mirrored to an Open Source (OSS) repository via **Copybara**.
However, not all files in this directory are exported.

### Internal-Only Files (NOT in OSS)

The following files are restricted to the Google-internal environment and **MUST
NOT** be referenced in any documentation that is intended for OSS users: -
`GEMINI.md` (This file) - `copy.bara.sky` - `deploy.sh` (Deployment script with
internal configurations) - `METADATA`, `OWNERS` - All `BUILD` and `BUILD.bazel`
files

### OSS-Exported Files

The primary codebase and its supporting documentation are exported: - `src/**`:
All source code. - `README.md`: The public-facing documentation. -
`pyproject.toml`, `uv.lock`, `.gitignore`. - `Dockerfile`: Production
container configuration. - `alembic/**`: Database migrations. - `scripts/**`:
Setup and utility scripts.

--------------------------------------------------------------------------------

## Open Source (OSS) & Copybara

This project is actively maintained as an Internal-First project that is
mirrored to Open Source.

### OSS Repository

The public version of this project is located at:
[github.com/looker-open-source/ca-demos-and-tools/tree/main/ca-agent-ops-prism](https://github.com/looker-open-source/ca-demos-and-tools/tree/main/ca-agent-ops-prism)

### Copybara Configuration

We use **Copybara** to synchronize changes from Google3 to GitHub. * **Config
File**: `copy.bara.sky` * **Workflow**: `prism_to_oss_local` * **What is
Exported**: * `src/**` (Core logic and UI) * `scripts/**` (Setup and utility
scripts) * `alembic/**` (Migrations) * `tests/**` (Test suite) * `README.md`,
`pyproject.toml`, `Dockerfile` * **What is Scrubbed**: * Internal Google IDs,
links, and infrastructure references are automatically removed. * License
headers (Apache 2.0) are prepended to all exported source files.

### Internal vs OSS Best Practices

While Copybara performs automated scrubbing, all contributors MUST follow these
rules: * **NO INTERNAL REFERENCES**: Do NOT use terms like "Cloud Top", "Borg",
"Stubby", or internal Google URLs in the codebase. * **NO PERSONAL DATA**: Avoid
including LDAPs or personal information in comments. * **ABSOLUTE PATHS**: Use
`google3/` relative paths for internal logic, but ensure they are compatible
with the OSS structure (Copybara handles the translation).
...
*   **uv Required**: Use `uv` for dependency management and execution.
    *   **Validate First**: Run `./scripts/run_tests.sh` to ensure all tests
        pass *before* requesting review or proceeding to the next task.
    *   **NO BLAZE/BUILD_CLEANER/PIP**: Do NOT use `blaze`, `build_cleaner`, or `pip`. This project does NOT use Bazel or Google3 build tools. It uses `uv` for everything.
*   **Database**: Use Docker for local development.
    `./scripts/setup_postgres.sh` will automatically manage a `postgres_local`
    container.
